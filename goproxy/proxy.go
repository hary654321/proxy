...
func (p *Proxy) transfer(src net.Conn, dst net.Conn) {
	log.Println("src", src.RemoteAddr().String())
	log.Println("dst", dst.RemoteAddr().String())
	go func() {
		buf := bufPool.Get().([]byte)
...
func CopyHeader(dst, src http.Header) {
	log.Println("CopyHeader dst", dst)
	log.Println(" CopyHeader  src", src)
	for k, vv := range src {
		for _, v := range vv {
			dst.Add(k, v)
		}
